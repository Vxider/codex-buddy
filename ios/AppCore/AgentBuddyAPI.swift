...
struct AgentBuddyServer: Codable, Identifiable, Equatable {
    let id: UUID
    var name: String
    var baseURL: String
...
final class AgentBuddyAPI {
    static let shared = AgentBuddyAPI()

    static let baseURLKey = "agentBuddyBaseURL"
    static let serversKey = "agentBuddyServers"
    static let selectedServerIDKey = "agentBuddySelectedServerID"
    static let infoPlistBaseURLKey = "AgentBuddyBaseURL"
    ...
    func loadServers(bundle: Bundle = .main) -> [AgentBuddyServer] {
        migrateBaseURLServerIfNeeded(bundle: bundle)
        guard let data = defaults.data(forKey: Self.serversKey) else {
            return []
        }
        return (try? decoder.decode([AgentBuddyServer].self, from: data)) ?? []
    }

    func currentServer(bundle: Bundle = .main) -> AgentBuddyServer? {
    ...
    func saveServer(name: String, baseURL: String, id: UUID? = nil, bundle: Bundle = .main) throws -> AgentBuddyServer {
        ...
        let server = AgentBuddyServer(id: id ?? UUID(), name: normalizedName, baseURL: normalizedURL)
    ...
    private func persistServers(_ servers: [AgentBuddyServer]) {
        guard let data = try? encoder.encode(servers) else { return }
        defaults.set(data, forKey: Self.serversKey)
    }

    private func migrateBaseURLServerIfNeeded(bundle: Bundle) {
        if defaults.data(forKey: Self.serversKey) != nil {
            return
        }

        let existingBaseURL = defaults.string(forKey: Self.baseURLKey)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !existingBaseURL.isEmpty, let normalized = try? normalizedBaseURL(existingBaseURL) else {
            return
        }

        let server = AgentBuddyServer(name: normalizedServerName("", baseURL: normalized), baseURL: normalized)
...
enum CodexAPIError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidBaseURL:
            return "Set a valid Agent Buddy base URL first."
        case .invalidServerURL:
