...
final class DeviceOutputController {
    static let bleEnabledKey = "codexBuddyBLELEDEnabled"
    static let usbEnabledKey = "codexBuddyUSBLEDEnabled"

    private let defaults: UserDefaults
    private let queue = DispatchQueue(label: "codex-buddy.device-outputs")
...
private final class USBSerialStatusPublisher {
    ...
    func publish(_ frame: StatusFrame) {
        ...
        if written == bytes.count {
            ...
            NSLog("CodexBuddy USB serial write failed on \(devicePath ?? "-"): wrote \(written) of \(bytes.count)")
            close()
        }
    }
    ...
    private func ensureOpen() -> Bool {
        ...
        guard let path = Self.detectDevice() else {
            NSLog("CodexBuddy USB serial device not found")
            return false
        }
        let nextFD = Darwin.open(path, O_WRONLY | O_NOCTTY | O_NONBLOCK)
        guard nextFD >= 0 else {
            NSLog("CodexBuddy USB serial open failed for \(path): errno \(errno)")
            return false
        }
        configure(fd: nextFD)
        fd = nextFD
        devicePath = path
        NSLog("CodexBuddy USB serial opened \(path)")
        return true
    }
    ...
    private func writeLine(_ line: String) {
        ...
        if written != bytes.count {
            NSLog("CodexBuddy USB serial command write failed on \(devicePath ?? "-"): wrote \(written) of \(bytes.count)")
            close()
        }
    }
...
private final class BLEStatusAdvertiser: NSObject, CBPeripheralManagerDelegate {
    ...
    private func advertiseLastFrame() {
        ...
        manager.startAdvertising([
            CBAdvertisementDataLocalNameKey: "CodexBuddy",
