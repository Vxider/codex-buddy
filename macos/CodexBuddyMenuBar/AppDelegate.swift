...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    private func rebuildMenu(_ menu: NSMenu, loading: Bool) {
        ...
        menu.addItem(.separator())
        menu.addItem(actionItem(title: "Open Codex-Buddy", action: #selector(openCodexBuddy(_:)), keyEquivalent: ""))
    ...
    @objc func openCodexBuddy(_ sender: Any? = nil) {
        statusItem.menu?.cancelTracking()
        DispatchQueue.main.async { [weak self] in
            self?.presentCodexBuddyWindow()
        }
    }

    private func presentCodexBuddyWindow() {
