...
func runSetup(args []string) int {
	...
	cfg.HookClient.AutostartCommand = []string{"systemctl", "--user", "start", "agent-buddy.service"}
...
func installArtifacts(cfg config.Config, resolvedConfigPath string, paths userPaths, skipSystemd bool) error {
	...
	if err := runCommand("systemctl", "--user", "enable", "--now", "agent-buddy.service"); err != nil {
