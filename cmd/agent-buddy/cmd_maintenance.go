...
func runDoctor(args []string) int {
	...
	checks := []struct {
		...
		{"legacy hooks", checkNoLegacyAgentBuddyHooks(paths.legacyHooksPath)},
	...
	if err := runCommand("systemctl", "--user", "is-active", "agent-buddy.service"); err != nil {
		failed = true
...
func runStart(args []string) int {
	...
	if hasServiceFile, err := hasSystemdServiceFile(); err == nil && hasServiceFile {
		...
		fmt.Printf("started systemd user service: agent-buddy.service\n")
		return 0
	}
	...
	fmt.Printf("started agent-buddy in background via %s\n", cfg.InternalBaseURL())
	return 0
}
...
func runRestart(args []string) int {
	...
	if hasServiceFile, err := hasSystemdServiceFile(); err == nil && hasServiceFile {
		...
		fmt.Printf("restarted systemd user service: agent-buddy.service\n")
		return 0
	}
	...
	fmt.Printf("restarted agent-buddy in background via %s\n", cfg.InternalBaseURL())
	return 0
}
...
func runStop(args []string) int {
	...
	if active, err := isSystemdServiceActive(); err == nil && active {
		if err := stopSystemdService(); err == nil {
			fmt.Printf("stopped systemd user service: agent-buddy.service\n")
			return 0
		}
...
func runUninstall(args []string) int {
	...
	var keepBinary bool
	fs.StringVar(&configPath, "config", "", "Path to agent-buddy JSON config")
	fs.BoolVar(&keepBinary, "keep-binary", false, "Keep ~/.local/bin/agent-buddy after uninstall")
	_ = fs.Parse(args)
	...
	_ = runCommand("systemctl", "--user", "disable", "--now", "agent-buddy.service")
	_ = os.Remove(paths.servicePath)
	_ = runCommand("systemctl", "--user", "daemon-reload")

	if err := removeAgentBuddyHooksConfig(paths.codexConfigPath); err != nil {
		fmt.Printf("remove hooks: %v\n", err)
		return 1
	}
	if err := removeLegacyAgentBuddyHooks(paths.legacyHooksPath); err != nil {
...
func runSystemdServiceCommand(action string) error {
	cmd := exec.Command("systemctl", "--user", action, "agent-buddy.service")
...
func isSystemdServiceActive() (bool, error) {
	cmd := exec.Command("systemctl", "--user", "is-active", "--quiet", "agent-buddy.service")
	if err := cmd.Run(); err != nil {
