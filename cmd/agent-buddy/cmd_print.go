...
func runPrintHooks(args []string) int {
	...
	if content == "" {
		fmt.Printf("read hooks: agent-buddy hooks are not configured in %s\n", paths.codexConfigPath)
		return 1
	}
	fmt.Print(content)
...
func runPrintClaudeHooks(args []string) int {
	fs := flag.NewFlagSet("print-claude-hooks", flag.ExitOnError)
	var configPath string
	fs.StringVar(&configPath, "config", "", "Path to agent-buddy JSON config")
	_ = fs.Parse(args)
	...
	content := claudeHooksSnippet(paths.binPath, resolvedConfigPath)
	fmt.Print(content)
