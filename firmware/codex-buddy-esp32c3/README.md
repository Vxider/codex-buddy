...
Run `codex-buddy` on the host and bridge its status stream to the ESP32-C3 serial
device:
...
```sh
codex-buddy esp32 --uart /dev/ttyACM0 --once
```
...
codex-buddy esp32 --uart /dev/ttyACM0 --motor 0
```
